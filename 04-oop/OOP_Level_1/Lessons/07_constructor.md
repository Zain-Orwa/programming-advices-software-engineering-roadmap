...
>A constructor has no return type because its purpose is not to return a value. Its purpose is
>to initialize the object that is currently being created.
2. When does the `clsAddress` constructor run?
>> When the object of that class is created. at this example: clsAddress* Pointer; // Usually no clsAddress object created yet // Therefore, no clsAddress constructor runs
3. Why does `clsAddress Address1;` fail in the original class?
>> clsAddress Address1; fails because it tries to call a default constructor with no arguments,
>> but the class only has a parameterized constructor that requires arguments. 
4. What is the difference between a default constructor and a parameterized constructor?
>> A default constructor can be called without arguments. It may be generated by the compiler
>> or written by the programmer. A parameterized constructor accepts arguments used to 
>> initialize the object. Neither constructor has a return type.
5. Why is a member initializer list preferred over assignment inside the constructor body?
>> A member initializer list is preferred because it initializes the data member directly
>> instead of initializing it first and then assigning another value inside the constructor
>> body. Initializer lists are also necessary for some members, such as const members and
>> references.
6. What happens if the arguments are passed in the wrong order?
>> If the arguments have the same types, the code may compile, but the values can go into 
>> the wrong members. That is a logical error.
>> If the argument types do not match, it is usually a compile-time error, One extra detail: 
>> C++ may sometimes convert types automatically, so even different types might compile 
>> but produce unexpected results.
7. Why must the constructor normally be under `public`?
>> the constructor is normally public so code outside the class can call it when creating an
>> object.
8. How many times does the constructor run when three different objects are created?
>> The constructor runs three times because three separate objects are created.
